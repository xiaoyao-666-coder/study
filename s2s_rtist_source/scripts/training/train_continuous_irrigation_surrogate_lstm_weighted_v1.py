#!/usr/bin/env python3
"""Train a high-regret weighted LSTM surrogate for irrigation decisions.

The plain pointwise LSTM has useful regression quality but still misses hard
non-collapse high-gain dates. The listwise ranker did not improve this. This
variant keeps the stable pointwise regression objective but increases loss
weight for high net-gain candidates and true best candidates.
"""
...
try:
    ...
    raise SystemExit(
        "PyTorch is required for train_continuous_irrigation_surrogate_lstm_weighted_v1.py."
    ) from exc
...
DEFAULT_OUT_DIR = (
    Path("site_general_surrogate_eval")
    / "continuous_ir_12site_10k_surrogate_lstm_weighted_loso_v1"
)
...
def make_weights(
    ...
    *,
    ...
    best_candidate_weight: float,
    noncollapse_best_weight: float,
    scale_quantile: float,
    max_weight: float,
) -> np.ndarray:
    y = pd.to_numeric(df[TARGET], errors="coerce").to_numpy(dtype=float)
    positive = np.maximum(y, 0.0)
    positives = positive[positive > 0]
    scale = float(np.quantile(positives, scale_quantile)) if len(positives) else 1.0
    if scale <= 1e-12:
        scale = 1.0

    weights = np.ones(len(df), dtype=float)
    weights += float(positive_gain_weight) * np.clip(positive / scale, 0.0, 1.0)

    if "is_best_ir" in df.columns:
        is_best = bool_series(df["is_best_ir"]).to_numpy(dtype=bool)
    else:
        best_idx = df.groupby("site_date_id")[TARGET].idxmax()
        is_best = np.zeros(len(df), dtype=bool)
        is_best[df.index.get_indexer(best_idx)] = True
    weights += float(best_candidate_weight) * is_best.astype(float)

    group_best = df.groupby("site_date_id")[TARGET].transform("max").to_numpy(dtype=float)
    noncollapse_best = is_best & (group_best > 1e-9)
    weights += float(noncollapse_best_weight) * noncollapse_best.astype(float)
    ...
    *,
    ...
    for _epoch in range(epochs):
        for seq_batch, tab_batch, y_batch, w_batch in loader:
            ...
            optimizer.zero_grad()
            err = model(seq_batch, tab_batch) - y_batch
            loss = (w_batch * err.square()).mean()
            loss.backward()
...
def main() -> None:
    ...
    parser.add_argument("--positive-gain-weight", type=float, default=4.0)
    parser.add_argument("--best-candidate-weight", type=float, default=6.0)
    parser.add_argument("--noncollapse-best-weight", type=float, default=8.0)
    ...
    weights_all = make_weights(
        df,
        positive_gain_weight=args.positive_gain_weight,
        best_candidate_weight=args.best_candidate_weight,
        noncollapse_best_weight=args.noncollapse_best_weight,
    ...
    pred_parts = []
    print(
        f"[weighted] loaded rows={len(df)} groups={len(groups)} seq_shape={seq_all.shape} tab_cols={len(tab_cols)}",
        flush=True,
    )
    for i, group in enumerate(groups):
        print(f"[weighted] fold {i + 1}/{len(groups)} holdout {args.cv_group_col}={group}", flush=True)
        ...
        part["model"] = "lstm_weighted"
        part["pred_net_gain_7d"] = preds
        pred_parts.append(part)
        print(f"[weighted] fold {i + 1}/{len(groups)} completed rows={int(test_mask.sum())}", flush=True)
    ...
    for site_date_id, part in pred_df.groupby("site_date_id", sort=False):
        ...
        true_rank = int(part["pred_net_gain_7d"].rank(method="min", ascending=False).loc[true_best.name])
        decisions.append(
            {
                ...
                "true_best_pred_net_gain": float(true_best["pred_net_gain_7d"]),
    ...
    metrics.update(
        {
            ...
            "best_candidate_weight": float(args.best_candidate_weight),
            "noncollapse_best_weight": float(args.noncollapse_best_weight),
    ...
    pred_path = out_dir / "continuous_irrigation_surrogate_lstm_weighted_v1_predictions.csv"
    decision_path = out_dir / "continuous_irrigation_surrogate_lstm_weighted_v1_decision_eval.csv"
    metrics_path = out_dir / "continuous_irrigation_surrogate_lstm_weighted_v1_metrics.csv"
    by_site_path = out_dir / "continuous_irrigation_surrogate_lstm_weighted_v1_by_site.csv"
    feature_path = out_dir / "continuous_irrigation_surrogate_lstm_weighted_v1_features.json"
    report_path = out_dir / "continuous_irrigation_surrogate_lstm_weighted_v1.md"
    ...
    lines = [
        "# Continuous Irrigation Surrogate LSTM Weighted V1",
        "",
        "## Scope",
        "",
        "- Site-general weighted pointwise LSTM over daily sequence features.",
        "- Higher loss weight for positive-gain and true-best candidates.",
    ...
    print("Continuous irrigation surrogate LSTM weighted v1")
