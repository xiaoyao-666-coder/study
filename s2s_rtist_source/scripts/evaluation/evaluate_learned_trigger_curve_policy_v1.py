...
DECISION_OUT = OUT_DIR / "learned_trigger_curve_policy_v1_decision_eval.csv"
METRICS_OUT = OUT_DIR / "learned_trigger_curve_policy_v1_metrics.txt"
...
def main() -> None:
    ...
    lines = [
        "Learned trigger + amount policy v1",
        "",
        f"samples: {SAMPLES}",
        f"tree predictions: {PRED}",
        "trigger model: numpy logistic classifier, leave-one-date-out",
        "threshold: tuned on training dates to minimize regret",
        "",
